...
run_rivet() {
  ...
  yoda="output_${lpt}_${spt}.yoda"

  cd $outputdir
  if [[ ! -f "$outputdir/LHC.yoda" ]]; then
    echo "$outputdir/LHC.yoda does not exist... This job might failed. abort."
    return 1
  fi
  rm -f $yoda

  #cp -f "$Hw_Loc/hw7_validation/FullShower/BL4/RAnalysis_final.cc" RAnalysis.cc
  #chmod 777 RAnalysis.cc
  ...
  cp -r /cms/ldap_home/taehee/HerwigWD/hw7_validation/FullShower/BL4/RivetAnalysis/preCompiled_${generation}_${lpt}_${spt}/Rivet.so .
...
if [[ "$generation" == "FO" ]]; then
  ...
  run_rivet $generation 32 13 $outputdir
  run_rivet $generation 52  5 $outputdir
