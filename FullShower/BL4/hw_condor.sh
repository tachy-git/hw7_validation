...
outputdir=/cms_scratch/taehee/HerwigSample/BL4_$com/hw_nEvt-${EVTpRUN}/MZp-${ZprimeMass}/${sample}/${process}
WD=$outputdir
...
elif [ "${ZprimeMass}" -eq 10 ] || [ "${ZprimeMass}" -eq 12 ] || [ "${ZprimeMass}" -eq 13 ] || [ "${ZprimeMass}" -eq 14 ]; then
  coupling="0.26666666666666666" #x4
elif [ "${ZprimeMass}" -eq 15 ]; then
  coupling="0.3" #x4.5
elif [ "${ZprimeMass}" -eq 20 ]; then
  coupling="0.3333333333333333" #x5
elif [ "${ZprimeMass}" -eq 25 ]; then
  coupling="0.36666666666666664" #x5.5
elif [ "${ZprimeMass}" -eq 30 ]; then
  coupling="0.4" #x6
elif [ "${ZprimeMass}" -eq 35 ] || [ "${ZprimeMass}" -eq 40 ]; then
  coupling="0.4666666666666667" #x7
elif [ "${ZprimeMass}" -eq 45 ]; then
  coupling="0.5333333333333333" #x8
...
mkdir -p ${WD}
cd ${WD}
cp /cms/ldap_home/taehee/HerwigWD/hw7_validation/FullShower/BL4/RAnalysis.cc .
cp -r /cms/ldap_home/taehee/HerwigWD/hw7_validation/FullShower/BL4/UFO/"MZp-${ZprimeMass}"/* .
...
sed -e "s/__NEVENTS__/${EVTpRUN}/g" ${Hw_Loc}/hw7_validation/FullShower/BL4/input/${ShowerSetting}.in > LHC.in
sed -i "s/__SEED__/${rnum}/g" LHC.in
sed -i "s/__COUPLING__/${coupling}/g" LHC.in
sed -i "s/__COM__/${com_}/g" LHC.in
sed -i "s/__DIR__/\/cms_scratch\/taehee\/HerwigSample\/BL4_$com\/mg_nEvt-${EVTpRUN}\/${sample}\/${process}/g" LHC.in
